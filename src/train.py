...
def training_loop(config, device, img_list, ann_list, train_transform, valid_transform):
    ...
    for fold, (train_index, valid_index) in enumerate(kf.split(img_list)):
        ...
        train_loader = torch.utils.data.DataLoader(train_set, batch_size=config['training']['batch'])
        valid_loader = torch.utils.data.DataLoader(valid_set, batch_size=config['training']['batch'])
        ...
        for epoch in range(num_epochs):
            ...
            train_loss = train_epoch(net,device, train_loader, criterion, optimizer)
            valid_loss = train_epoch(net,device, valid_loader, criterion, optimizer)
            ...
            #     # Saving State Dict
            #     torch.save(net.state_dict(), config['folders']['save_checkpoint']+config['training']['checkpoint_name']+'.pt')
        ...
        for name, net in net.named_children():  
            try:
                ...
                print('could not reset ', name)
    
    loss_dictionary  = fold_performance(config, foldperf)

    return loss_dictionary



def train_epoch(net,device,data_loader,loss_fn,optimizer):
    train_loss=0.0
    net.train()
...
def valid_epoch(net,device,dataloader,loss_fn):
    ...
    for data in dataloader:
        ...
        output = net(image)
        loss=loss_fn(output,keypoints)
        valid_loss+=loss.item() # TODO: multiply by image size?
    return valid_loss

def fold_performance(config, foldperf):
    train_loss, valid_loss=[],[]
    k = config['training']['kfold']
    for f in range(1,k+1):
        train_loss.append(np.mean(foldperf['fold{}'.format(f)]['train_loss']))
        valid_loss.append(np.mean(foldperf['fold{}'.format(f)]['valid_loss']))

    print('Performance of {} fold cross validation'.format(k))
    print("Average Training Loss: {:.5f} \t Average Test Loss: {:.5f}".format(np.mean(train_loss),np.mean(valid_loss)))

    loss_dictionary = {"processing": config['processing'], "training":config['training'], "train_folds": train_loss, "valid_folds": valid_loss}

    utils.dump_to_json(loss_dictionary,os.path.join(config['folders']['save_losses'],config['training']['checkpoint_name']+".json"))
